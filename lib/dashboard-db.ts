...
/** 공정 데이터용 메인 테이블. simulation_results 고정 (PROCESS_TABLE_NAME으로 변경 가능) */
...
export async function getProcessColumnMap(
  ...
  const quantityCol = findColumn(columns, ['quantity', 'amount', 'count', 'qty', 'output', 'production', 'production_amount', '생산', '수량']);
